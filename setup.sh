...
select ans in "plexpass" "plexserver" "sonarr" "radarr" "jackett"  "transmission" "Quit"; do
    case $ans in
        ...
        sonarr ) /usr/local/bin/zsh install-sonarr.sh; break;;
        radarr ) /usr/local/bin/zsh install-radarr.sh; break;;
	    jackett ) /usr/local/bin/zsh install-jackett.sh; break;;
        transmission ) /usr/local/bin/zsh install-transmission.sh; break;;
	    Quit ) exit;;
    esac
done
