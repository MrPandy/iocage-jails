...
iocage create -n "jackett" -r 11.2-RELEASE ip4_addr="vnet0|$jackettip/24" defaultrouter=$gwip vnet="on" allow_raw_sockets="1" boot="on"

echo '{"pkgs":["nano","mono","curl"]}' > /tmp/pkg.json
iocage create -n "jackett" -p /tmp/pkg.json -r 11.2-RELEASE ip4_addr="vnet0|10.0.0.10/24" defaultrouter="10.0.0.1" vnet="on" allow_raw_sockets="1" boot="on"
rm /tmp/pkg.json
